...
This project is a fully-automated statistical analysis platform for the "Melate Retro" lottery, designed to provide users with data-driven insights that surpass simple random selection. The core system is a highly efficient data processing pipeline built on GitHub Actions, automating web scraping (`firebase_scraper.py`), pre-computing statistical models (`precompute_analysis.py`), and running brute-force analysis (`brute_force_analyzer.py`). The analysis engine now comprehensively analyzes all 7 numbers drawn in each contest (6 natural + 1 additional) to maximize the probability of hitting both main and secondary prizes. Results are served to a modular JavaScript and Tailwind CSS frontend via a Firebase backend (Firestore, Authentication, Hosting). Key user-facing features include an interactive dashboard, a 9-rule weighted engine, and tools to generate, rate, and backtest potential plays. The architecture will be refactored to improve maintainability and scalability, addressing potential vendor lock-in concerns.
